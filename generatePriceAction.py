...
def generate_crypto_price_action(days, volatility=0.5, drift=0.1, seed=None):
    ...
    return prices, initial_price, sma_20, rsi_14

# Generate price data for a crypto asset
num_days = 365  # Number of days
seed_value = 42  # Seed for replication
price_data, initial_price, sma_20, rsi_14 = generate_crypto_price_action(num_days, volatility=0.8, drift=0.15, seed=seed_value)
...
ax1.plot(range(0,len(price_data)), price_data, label='Price')
ax1.plot(range(0,len(price_data)), sma_20, label='SMA (20)')
...
ax2.plot(range(0,len(price_data)), rsi_14, color='orange', label='RSI (14)')
